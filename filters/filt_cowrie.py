...
from tahoe import MongoBackend, NoBackend, Instance, \
     Attribute, Object, Event, Session, parse
...
def filt_cowrie(backend=_BACKEND):
    try:
        filt_id = "filter--644673c8-3602-4114-a286-f4caec2ef5c8"
        
        query = {"itype":"raw", "sub_type":"unr_honeypot",
                 "filters":{"$ne":filt_id},
                 "_valid" : {"$ne" : False}}

        cursor = backend.find(query, _PROJECTION, no_cursor_timeout=True)
        if not cursor:
            logging.error("filt_cowrie: This should not happen")

        any_success = False
        for raw in cursor:
            try:
                try:
                    data_body = json.loads(raw["data"]["message"])
                    raw["data"].update(data_body)
                    if "eventid" not in raw["data"]:
                        continue
                except:
                    logLine = "\nfilt_cowrie 0: _hash: {}, timestamp: {}\n".format(raw["_hash"]
                                                                    ,raw["data"]["@timestamp"])
                    logging.error(logLine, exc_info=True)
    ##                backend.update_one( {"_hash" : raw["_hash"]}, {"$set" : {"_valid" : False}})
                ...
                if   eventid == "cowrie.client.kex": j = ClientKex(raw)  
                ...
                elif eventid == "cowrie.command.failed": j = CommandFailed(raw)
                elif eventid == "cowrie.command.input": j = CommandInput(raw)
                elif eventid == "cowrie.command.success": j = CommandSuccess(raw)
                    ...
                    logging.warning("filt_cowrie 1: Unknown eventid: " + eventid)
            except (KeyboardInterrupt, SystemExit): raise
            except:
                logging.error("\nfilt_cowrie 2: eventid: " + eventid +
                              ", timestamp: " + timestamp + "\n", exc_info=True)
##                backend.update_one( {"_hash" : raw["_hash"]}, {"$set" : {"_valid" : False}})
                j = False
            else: 
                backend.update_one({"_hash":raw["_hash"]}, {"$addToSet":{"filters":filt_id}})
        ...
        logging.error("filt_cowrie 3: ", exc_info=True)
        return False
    return any_success
...
class Cowrie():
    def __init__(self):
        ...
        raw = parse(self.raw, backend=_BACKEND)
        ref_hash_list = e.related_hash() + [session._hash]
        raw.add_ref(ref_hash_list)
...
class CommandInput(Cowrie):
    def __init__(self, raw, objects=[]):
        ...
        self.raw_data, self.raw = raw.pop("data"), raw
        command_att = Attribute('shell_command', self.raw_data["input"])
